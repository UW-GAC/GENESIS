...
test_that("filterMonomorphic - discrete", {
    ...
    het <- rep(1, nsamp)
    alt <- rep(2, nsamp)
    ref.miss <- ref; ref.miss[sample(nsamp, 5)] <- NA
    het.miss <- het; het.miss[sample(nsamp, 5)] <- NA
    alt.miss <- alt; alt.miss[sample(nsamp, 5)] <- NA
    ok <- sample(rbinom(nsamp, 1, 0.2))
    ok.miss <- ok; ok.miss[sample(nsamp, 5)] <- NA
...
test_that("filterMonomorphic - imputed", {
    nsamp <- 100
    ref <- rep(0, nsamp) + runif(nsamp, 0, 1e-9)
    het <- rep(1, nsamp) + runif(nsamp, -1e-9, 1e-9)
    alt <- rep(2, nsamp) - runif(nsamp, 0, 1e-9)
    ref.miss <- ref; ref.miss[sample(nsamp, 5)] <- NA
    het.miss <- het; het.miss[sample(nsamp, 5)] <- NA
    alt.miss <- alt; alt.miss[sample(nsamp, 5)] <- NA
    ok <- runif(nsamp, 0, 2)
