...
test_that("returns expected names", {
  ...
  expect_equal(names(jointmod), c("Stat.joint", "pval.joint", "pve.joint", "fixef", "betaCov"))
})
...
test_that("returns expected types", {
  ...
  expect_is(jointmod$pve.joint, "numeric")
...
test_that("works with one snp", {
  ...
  expect_equal(length(jointmod$pve.joint), 1)
  ...
  expect_equal(jointmod$pve.joint, jointmod$fixef$pve)
