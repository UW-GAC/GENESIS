...
The next step is PC-AiR, in which we select a set of unrelated samples that is maximally informative about all ancestries in the sample, use this unrelated set for Principal Component Analysis (PCA), then project the relatives onto the PCs.  We use a kinship threshold of degree 3 (unrelated is less than first cousins). In this example, we use the KING estimates for both kinship (`kinMat`) and ancestry divergence (`divMat`). KING kinship estimates are negative for samples with different ancestry.

```{r pcair}
pcs <- pcair(seqData, kinMat=kingMat, kin.thresh=2^(-9/2),
                      divMat=kingMat, div.thresh=-2^(-9/2),
             snp.include=pruned, snp.block.size=22000)
```
...
The first 2 PCs separate populations, so we use them to compute kinship estimates adjusting for ancestry.

```{r pcrelate}
pcrel <- pcrelate(seqData, pcMat=pcs$vectors[,1:2], training.set=pcs$unrels,
                  snp.include=pruned, snp.block.size=22000)
```
...
To improve our estimates for PCs and kinship, we could run another iteration of PC-AiR and PC-Relate, this time using the PC-Relate kinship estimates as the `kinMat` argument to `pcair`.  The KING matrix is still used for ancestry divergence. We could then use those new PCs to calculate revised kinship estimates.
...
# find variants that overlap with each transcript
txdb <- TxDb.Hsapiens.UCSC.hg19.knownGene
gr <- renameSeqlevels(gr, paste0("chr", seqlevels(gr)))
ts <- transcriptsByOverlaps(txdb, gr, columns="GENEID")
ts <- reduce(ts)
ts <- renameSeqlevels(ts, sub("chr", "", seqlevels(ts)))

# create an iterator where each successive unit is a different transcript
iterator <- SeqVarRangeIterator(seqData, variantRanges=ts, verbose=FALSE)

# do a burden test on the rare variants in each transcript
