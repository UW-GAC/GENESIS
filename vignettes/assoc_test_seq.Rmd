...
The first 2 PCs separate populations, so we use them to compute kinship estimates adjusting for ancestry. The \code{pcrelate} method requires creating a SeqVarBlockIterator object, which should iterate over the pruned SNPs only.

```{r pcrelate}
seqSetFilter(seqData, variant.id=pruned)
iterator <- SeqVarBlockIterator(seqData, variantBlock=20000)
