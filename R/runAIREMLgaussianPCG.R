.runAIREMLgaussianPCG <- function(Y, X, start, covMatList, group.idx, AIREML.tol, drop.zeros, max.iter, verbose, Method = "EST"){
    # initial values
    startt <- Sys.time()
    m <- length(covMatList)
    #print(m)
    ...
    repeat({
        ...
        #print current REML
        #REMLEXT[reps] <<- .calREML(Y, X, Sigma, Sigma.inv_X, PY)
        #print(Sys.time())
        ...
        if(reps > 1){
            ...
            if(max(abs(sigma2.k - sigma2.kplus1)/(abs(sigma2.k) + abs(sigma2.k) + 0.02)) < 0.02){
              print(paste0("Finished, the final var component is",sigma2.kplus1))
