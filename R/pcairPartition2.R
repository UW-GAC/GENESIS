pcairPartition2_gds <- function(kinobj, divobj,
	...
	div.id.all <- read.gdsn(index.gdsn(divobj, 'sample.id'))
	...
	rellist <- apply.gdsn(	node = index.gdsn(kinobj, 'kinship'), margin = 2, 
	...
	kinsum <- apply.gdsn(	node = index.gdsn(kinobj, 'kinship'), margin = 2, 
	...
	message('Identifying pairs of divergent samples using divergence threshold ', div.thresh)
	...
	if(!is.null(unrel.set)){
		message('Forcing samles specified in unrel.set into the unrelated set')
	...
	message('Paritioning samples into unrelated and related sets...')
	# iterate
	iter <- 0
	while(length(nrel) > 0){
		iter <- iter + 1
		if(iter %% 1000 == 0) message('    ...', iter, ' samples added to related.set...')
...
pcairPartition2_matrix <- function(kinobj, divobj,
							kin.thresh = 2^(-11/2),
							div.thresh = -2^(-11/2),
							unrel.set = NULL){

	if(is.null(colnames(kinobj)) | is.null(colnames(divobj))){
		stop('sample.ids must be present in colnames of kinobj and divobj when they are matrices')
	}

	# sample.id from kinship matrix
	kin.id.all <- colnames(kinobj)
	# sample.id from divergence matrix
	div.id.all <- colnames(divobj)
	...
	kin.id <- kin.id.all[kin.read]
	# create list of relatives for each sample
	rellist <- apply(	kinobj[kin.read, kin.read], MARGIN = 2,
						FUN = function(x){ kin.id[x > kin.thresh]})
	names(rellist) <- kin.id
	# remove self from rellist
	for(i in 1:length(rellist)){
	...
	kin.id <- kin.id.all[kin.read]
	# compute 'total kinship' values
	kinsum <- apply(kinobj[kin.read, kin.read], MARGIN = 2,
					FUN = function(x){ sum(x[x > kin.thresh]) })
	names(kinsum) <- kin.id
	kinsum <- unlist(kinsum)
